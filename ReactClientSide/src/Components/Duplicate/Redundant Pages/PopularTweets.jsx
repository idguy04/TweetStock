    ...
    fetch("https://localhost:44384/api/Twitter?user_id=" + userId, {
      ...
      .then(
        (result) => {
          ...
          console.log("fetch res= ", user_tweets);
          let likesCounter = 0, rtCounter = 0, userEng = 0;;
          for (var i = 0; i < user_tweets.length; i++) {
            likesCounter += user_tweets[i].favorite_count;
            rtCounter += user_tweets[i].retweet_count;
          }
          userEng = ((likesCounter + rtCounter) / user_tweets[0].user.followers_count) / user_tweets.length;
          console.log(userEng);
          setTweetsArr(oldArr => [...oldArr, renderTweet(tweetId, userId, userEng)]);
        },
        (error) => {
