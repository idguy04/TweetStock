...
export default function PredTable(props) {
        ...
        if (key2==='sentiment'){
          if(tweet[key][key2].hasOwnProperty('pos')){
            Object.keys(tweet[key][key2]).forEach(key3=>{
              ...
              sentiment_columns.push({Header:key3.charAt(0) + key3.slice(1),accessor:key3})
            })
          } else {
            sentiment_data[0][key2] = tweet[key][key2]
            sentiment_columns.push({Header:key2.charAt(0) + key2.slice(1),accessor:key2})
            only_compound_stat = true
          }

        } else {
          tweet_data[0][key2] = tweet[key][key2]
          tweet_columns.push({Header:key2.charAt(0) + key2.slice(1),accessor:key2})

        }
      })
    } else if (key==='user_engagement') {
        user_data[0][key] = tweet[key].toFixed(2)
        user_columns.push({Header:key.charAt(0) + key.slice(1),accessor:key})
    }
  })
  ...
  return (
    <div style={{ display: "grid" }}>
      ...
      <ReactTable columns={only_compound_stat ? sentiment_columns : sent_cols} data={sentiment_data}></ReactTable>
